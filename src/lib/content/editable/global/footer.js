/*
  Edit this file for global footer content.
  Footer logos and copyright text are shown at the bottom of every page.
*/
export const footer = {
  logos: [
    {
      src: '/logos/EU-logo.png',
      alt: {
        en: 'European Union logo',
        uk: 'Логотип Європейського Союзу',
        ro: 'Sigla Uniunii Europene',
        hy: 'Եվրոպական միության լոգո'
      },
      width: '185px'
    },
    {
      src: '/logos/EU4Green-logo.png',
      alt: {
        en: 'EU4Green logo',
        uk: 'Логотип EU4Green',
        ro: 'Sigla EU4Green',
        hy: 'EU4Green լոգո'
      },
      width: '72px'
    }
  ],
  copyright: {
    en: '© 2026 EU4 GreenRecovery East programme. All rights reserved. Licensed to the European Union under conditions.',
    uk: '© 2026 Програма EU4 GreenRecovery East. Усі права захищено. Ліцензовано для Європейського Союзу відповідно до умов.',
    ro: '© 2026 Programul EU4 GreenRecovery East. Toate drepturile rezervate. Licențiat Uniunii Europene conform condițiilor.',
    hy: '© 2026 EU4 GreenRecovery East ծրագիր։ Բոլոր իրավունքները պաշտպանված են։ Լիցենզավորված է Եվրոպական միությանը՝ պայմանների համաձայն։'
  },
  disclaimer: {
    en: 'This website is funded by the European Union. Its content are the sole responsibility of the EU4Green Recovery East programme, financed by the European Union. The content of the website belongs to the authors and does not necessarily reflect the views of the European Union, its members or the Governments of the Eastern Partnership Countries. This website and any content included herein are without prejudice to the status of, or sovereignty over, any territory, to the delimitation of international frontiers and boundaries, and to the name of any territory, city or area. Furthermore, neither the EU nor the EU4Green Recovery East programme are liable for risks arising from the use of this website or any third-party tools.',
    uk: 'Цей вебсайт фінансується Європейським Союзом. Його зміст є виключною відповідальністю програми EU4Green Recovery East, що фінансується Європейським Союзом. Зміст вебсайту належить авторам і не обов’язково відображає погляди Європейського Союзу, його держав-членів або урядів країн Східного партнерства. Цей вебсайт і будь-який розміщений на ньому вміст не впливають на статус чи суверенітет будь-якої території, визначення міжнародних кордонів і меж або назву будь-якої території, міста чи місцевості. Крім того, ні ЄС, ні програма EU4Green Recovery East не несуть відповідальності за ризики, що виникають унаслідок використання цього вебсайту або будь-яких сторонніх інструментів.',
    ro: 'Acest site web este finanțat de Uniunea Europeană. Conținutul său este responsabilitatea exclusivă a programului EU4Green Recovery East, finanțat de Uniunea Europeană. Conținutul site-ului aparține autorilor și nu reflectă neapărat opiniile Uniunii Europene, ale statelor sale membre sau ale guvernelor țărilor Parteneriatului Estic. Acest site web și orice conținut inclus aici nu aduc atingere statutului sau suveranității niciunui teritoriu, delimitării frontierelor și granițelor internaționale sau denumirii vreunui teritoriu, oraș ori zonă. De asemenea, nici UE, nici programul EU4Green Recovery East nu sunt răspunzătoare pentru riscurile care decurg din utilizarea acestui site web sau a oricăror instrumente terțe.',
    hy: 'Այս կայքը ֆինանսավորվում է Եվրոպական միության կողմից։ Դրա բովանդակության համար պատասխանատվություն է կրում բացառապես Եվրոպական միության կողմից ֆինանսավորվող EU4Green Recovery East ծրագիրը։ Կայքի բովանդակությունը պատկանում է հեղինակներին եւ պարտադիր չէ, որ արտահայտի Եվրոպական միության, նրա անդամների կամ Արեւելյան գործընկերության երկրների կառավարությունների տեսակետները։ Այս կայքը եւ այստեղ ներառված ցանկացած բովանդակություն չեն կանխորոշում որեւէ տարածքի կարգավիճակը կամ ինքնիշխանությունը, միջազգային սահմանների եւ սահմանագծերի սահմանումը կամ որեւէ տարածքի, քաղաքի կամ շրջանի անվանումը։ Բացի այդ, ո՛չ ԵՄ-ն, ո՛չ EU4Green Recovery East ծրագիրը պատասխանատվություն չեն կրում այս կայքի կամ որեւէ երրորդ կողմի գործիքների օգտագործումից առաջացող ռիսկերի համար։'
  }
};
